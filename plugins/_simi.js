...
handler.before = async (m) => {
  ...
  if (chat.simi) {
    ...
    try {
      ...
      let ressimi = await fetch(`https://api.simsimi.net/v2/?text=${encodeURIComponent(m.text)}&lc=pt`)
      let data = await ressimi.json();

      if (data.success == 'Não entendi o que você disse. Por favor, me ensine.') {
        return m.reply(`${lol}`)
      }

      await m.reply(data.success)
    } catch {
      if (textodem.includes('Olá')) textodem = textodem.replace('Olá', 'Hello')
      if (textodem.includes('olá')) textodem = textodem.replace('olá', 'hello')
      if (textodem.includes('OLÁ')) textodem = textodem.replace('OLÁ', 'HELLO')
